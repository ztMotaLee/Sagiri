...
class DarkEnhanceDataset(data.Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, np.ndarray]:
        ...
        # print(f"lq max: {lq_img.max()}, min: {lq_img.min()}")
        gt_img = gt_img * 2 - 1###-1,1;lq:0,1?
        mask = np.zeros_like(lq_img)
        mask[(lq_img == 0) | (lq_img == 1)] = 1
        ##all zero.
        mask = mask.astype(np.float32)
        return {'hint': lq_img, 'jpg': gt_img, 'txt':caption, 'mask_key': mask}
