...
def remove_prefix_from_state_dict(state_dict, prefix):
    keys_to_delete = [key for key in state_dict if key.startswith(prefix)]
    for key in keys_to_delete:
        del state_dict[key]
    return state_dict
# def load_state_dict(model: nn.Module, state_dict: Mapping[str, Any], strict: bool=False, remove_prefix="cond_encoder.encoder") -> None:
def load_state_dict(model: nn.Module, state_dict: Mapping[str, Any], strict: bool=False, remove_prefix=None) -> None:
    state_dict = state_dict.get("state_dict", state_dict)
  
    if remove_prefix:
        state_dict = remove_prefix_from_state_dict(state_dict, remove_prefix)
