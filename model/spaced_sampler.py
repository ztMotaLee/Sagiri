...
class SpacedSampler:
    ...
    def q_sample(
        ...
        return (
            ...
            * noise ##mathematical transformation
        )
    ...
    def sample(
        ...
        for i, step in enumerate(iterator):
            ...
            img, x0_partial = self.p_sample(
                img, cond, ts, index=index,##time step
                cfg_scale=cfg_scale, uncond=uncond,
                cond_fn=cond_fn
            )
            if mask is not None:
                # print("mask exist.")
                # print("mask_latent shape:", mask_latent.shape, "Max:", mask_latent.max().item(), "Min:", mask_latent.min().item())
                # print("img shape:", img.shape, "Max:", img.max().item(), "Min:", img.min().item())
                img_orig = self.model.q_sample(cond['c_latent'][0], ts)
                # print("img_orig shape:", img_orig.shape, "Max:", img_orig.max().item(), "Min:", img_orig.min().item())
                img = img * mask_latent + (1. - mask_latent) * img_orig#img:16,4,64,64
                # if i % 10 == 0:
                # img = self.resample_step(img, cond, uncond, ...)
            # TODO: Resampling strategy.
            # if i % log_every_t == 0 or i == timesteps - 1:
            #     intermediates.append(x0_partial)
        
        img_pixel = (self.model.decode_first_stage(img) + 1) / 2
        # img_pixel = img_pixel * mask + (1. - mask) * cond_img
        # apply color correction (borrowed from StableSR)
        if color_fix_type == "adain":
