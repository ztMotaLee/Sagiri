...
def fft_loss(pred, label):
    fft_pred = torch.fft.fft2(pred)
    fft_label = torch.fft.fft2(label)
    return torch.mean(torch.abs(fft_pred - fft_label))
def histogram_loss(pred, label, bins=256):
    min_val, max_val = 0, 1
    pred_hist = torch.histc(pred, bins=bins, min=min_val, max=max_val)
    label_hist = torch.histc(label, bins=bins, min=min_val, max=max_val)

    pred_hist = pred_hist / pred_hist.sum()
    label_hist = label_hist / label_hist.sum()

    hist_loss = F.l1_loss(pred_hist, label_hist, reduction='sum')
    return hist_loss
class SwinIR(pl.LightningModule, ImageLoggerMixin):
    ...
    def get_loss(self, pred: torch.Tensor, label: torch.Tensor) -> torch.Tensor:
        """
        Compute combined loss between model predictions and labels.
        ...
        Returns:
            loss (torch.Tensor): The combined loss tensor.
        """
        # MSE Loss
        mse_loss = F.mse_loss(input=pred, target=label, reduction="sum")*0.0001

        # Histogram Loss
        hist_loss = histogram_loss(pred, label)*10
        ...
        # print(f"value range is from {min_val} to {max_val}")
        fftl = fft_loss(pred, label)
        print('mse_loss:'+str(mse_loss))
        print('hist_loss:'+str(hist_loss))
        print('fftloss:'+str(fftl))
        combined_loss = mse_loss + hist_loss + fftl
