...
from ldm.models.diffusion import ddpm_mask as ddpm_mask
from ldm.models.diffusion.ddpm_mask import LatentInpaintDiffusion
...
def print_structure(d, indent=0):
    for key, value in d.items():
        ...
        if isinstance(value, dict):
            ...
            print('List[Length: {}]'.format(len(value)))
...
class ControlLDM(LatentInpaintDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        # print("enter cldm.get_input")
        x, c = super().get_input(batch, self.first_stage_key, *args, **kwargs)
        # x, c = super(LatentInpaintDiffusion, self).get_input(batch, k, self.first_stage_key, *args, **kwargs)
        control = batch[self.control_key]
        mask=batch[self.mask_key]
        if bs is not None:
            control = control[:bs]#16,512,512,3
            mask = mask[:bs]#16,512,512,3
        # control = control*(1-mask)
        ...
        lq = control
        ############################
        ##TODO: Firstly in inpainting, we directly add mask in control. The method is that we can 
        # apply preprocess model
        # apply condition encoder
        c_latent = self.apply_condition_encoder(control)
        mask_latent = mask
        return x, dict(c_crossattn=[c], c_latent=[c_latent], lq=[lq], c_concat=[control],mask=[mask],mask_latent=[mask_latent])

    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        assert isinstance(cond, dict)
        diffusion_model = self.model.diffusion_model
        # print(type(cond['c_crossattn']))
        # print(cond['c_crossattn'])
        # cond_txt = torch.cat(cond['c_crossattn'], 1)
        # print_structure(cond)
        try:
            cond_txt = torch.cat(cond['c_crossattn'][0]['c_crossattn'], dim=1)
        except:
            # print_structure(cond)
            cond_txt = torch.cat(cond['c_crossattn'], 1)
    ...
    def log_images(self, batch, sample_steps=50, *args, **kwargs):
        log = dict()
        # log = super(LatentInpaintDiffusion, self).log_images(batch, *args, **kwargs)
        z, c = self.get_input(batch, self.first_stage_key)
        c_lq = c["lq"][0]
        c_latent = c["c_latent"][0]
        mask=c['mask'][0]
        mask_latent=c['mask_latent'][0]
        ...
        # log["mask"] = mask
        return log
#enter inpaint_get_input
# enter latentdiffusion_getinput
    @torch.no_grad()
    def sample_log(self, cond_img, steps, mask, mask_latent):
        ...
        mask_latent_mean = mask_latent_resized.mean(dim=1, keepdim=True)
        mask_latent = torch.cat([mask_latent_resized, mask_latent_mean], dim=1)

        samples = sampler.sample(
            steps, shape, cond_img, mask, mask_latent, positive_prompt="", negative_prompt="",
